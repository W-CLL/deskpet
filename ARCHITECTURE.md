...
5. 同一安装 ID 和凭据可以安全重试。第二台新设备可再填同一组已用购买码，加入原账号，每账号最多 2 台有效授权。
6. 客户端后续使用 `Bearer <licenseId>.<credential>`，服务端由授权内部解析 `accountId`，客户端不能指定数据账号。
7. 管理员可生成绑定已有账号的一次性换机码；新设备绑定成功后该账号全部有效授权自动撤销。
8. 管理员撤销设备授权后，该设备后续鉴权立即失败；账号数据仍归属于原 `accountId`。同一账号的来访按设备回执，两台都能领到同一封 GIF，直到过期。
