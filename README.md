...
这是一个可独立部署的 Express 5 应用，为桌搭子提供管理后台、版本发布、更新下载、一次性激活码、设备授权、账号互动统计、互动内容库、体验来访表情和用户反馈管理。

服务器只接收本地 Windows 电脑已经打包完成的 EXE，不负责编译 C#、运行 Inno Setup 或构建桌面应用。

## 2026-09-18 本地产品改进

此工作树包含尚未部署的产品改进，需要配套新版客户端验收：

- 有效体验与正式激活均可主动加入大厅并向在线用户发送 GIF；私人搭子绑定与私密发送仍需正式激活。新档案默认不加入大厅，已有账号选择保留。
- 体验后激活衔接大厅昵称、加入状态、待收投递与设备回执；加入已有账号时以该账号原有昵称和大厅设置为准。
- 检查更新 `/api/update/latest` 不进入使用量采集。历史接口排行和接口总数也排除此路径，真实安装包下载继续统计。旧的单设备聚合总量无法精确拆回，不伪造历史回减。
- 当前 Windows、macOS、Android 可以获取六类互动内容；新互动账号以当前远程默认初始化，已有用户选择不被远程默认覆盖。
- 已发布/草稿依据 `publishedAt` 统计；管理后台补充撤销授权、换机、设备收取回执及用户更新说明的解释。

检查命令为 `node scripts/check-js.js` 与 `node --test test/*.test.js`。测试使用独立临时数据，不需要操作生产账号。完整三端对照与手动验收步骤见相邻客户端仓库 `zhuodazi/docs/product-experience/2026-09-18-acceptance.md`。
...
`licenseId`。一组普通购买码最多同时激活 2 台设备，第二台填同一组码加入原账号。
旧客户端继续使用原来的 Bearer 凭据；已有设备授权在升级后会自动建立账号，不要
求用户重新输入激活码。管理后台可以为已有账号生成 24 小时有效的一次性换机码，
新设备绑定成功后该账号全部有效授权自动撤销，账号数据保持不变。来访按设备回执，
同一账号的电脑和手机都能领到对方发来的 GIF。
...
| `DESKPET_BOOTSTRAP_VERSION` | `2.5.7` | Windows x64 对外公开的稳定安装包版本回退值；发布版本后以 `publicVersions` 为准 |
| `DESKPET_MACOS_BOOTSTRAP_VERSION` | `2.2.5` | macOS 两种架构对外公开的稳定安装包版本回退值 |
