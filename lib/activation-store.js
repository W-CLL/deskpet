...
class ActivationStore {
  ...
  activate({
    ...
    try {
      ...
      if (codeRecord.status === 'used') {
        ...
        const account = existing && this.database.prepare(`
      ...
      this.database.exec('COMMIT');
      return { accountId, licenseId, activatedAt, alreadyActivated: false };
    } catch (error) {
  ...
  trialStatus({ installationId, credential, appVersion = '' } = {}) {
    ...
    try {
      ...
      if (!record) {
        ...
        this.database.prepare(`
          UPDATE trials SET last_seen_at = ?, app_version = ?
          WHERE installation_id = ?
        `).run(nowAt, safeText(appVersion, 40), normalizedInstallationId);
      }
      this.database.exec('COMMIT');
      return {
        allowed: Date.parse(record.expires_at) > now,
        remainingSeconds: Math.max(0, Math.ceil((Date.parse(record.expires_at) - now) / 1000)),
  ...
  list() {
    ...
    const rows = this.database.prepare(`
      SELECT c.id, c.code_suffix, c.code_ciphertext IS NOT NULL AS can_reveal,
        ...
        l.app_version, l.platform, l.architecture, l.created_at AS license_created_at,
        l.revoked_at, l.last_update_at,
        l.revocation_reason, COALESCE(l.account_id, c.target_account_id) AS account_id,
      ...
      LEFT JOIN accounts a ON a.id = COALESCE(l.account_id, c.target_account_id)
    ...
    for (const item of codes) {
      ...
      if (item.license) {
        const platform = Object.hasOwn(summary.platforms, item.license.platform)
          ? item.license.platform
          : 'unknown';
        summary.platforms[platform].total += 1;
        if (item.license.status === 'active') summary.platforms[platform].active += 1;
        if (item.license.status === 'revoked') summary.platforms[platform].revoked += 1;
      }
    }
