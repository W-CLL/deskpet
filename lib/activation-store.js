...
class ActivationStore {
  ...
  deviceInventory() {
    const licenses = this.database.prepare(`
      SELECT l.id, l.account_id, l.installation_id, l.status, l.app_version, l.platform,
        l.architecture, l.created_at,
        COALESCE(l.last_update_at, l.created_at) AS last_seen_at,
        c.id AS activation_code_id, c.code_suffix, c.code_ciphertext, c.used_at
      FROM licenses l
      LEFT JOIN activation_codes c ON c.id = l.activation_code_id
      ORDER BY l.created_at DESC
    `).all().map((row) => {
      let activationCode = null;
      if (row.code_ciphertext) {
        try {
          activationCode = this.decryptCode(row.code_ciphertext);
        } catch {
          activationCode = null;
        }
      }
      return {
        ...
        deviceCode: String(row.installation_id).slice(-12),
        ...
        activatedAt: row.used_at || row.created_at,
        lastSeenAt: row.last_seen_at,
        activationCodeId: row.activation_code_id || null,
        maskedCode: row.code_suffix ? `****${row.code_suffix}` : null,
        activationCode
      };
    });
