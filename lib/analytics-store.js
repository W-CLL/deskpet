...
const { isUpdateCheckPath } = require('./usage-policy');
...
class AnalyticsStore {
  ...
  recordRequest(event) {
    if (isUpdateCheckPath(event.path)) return;
  ...
  usageDetails() {
    ...
    const apiRequestTotal = Number(this.database.prepare(`
      SELECT COALESCE(SUM(request_count), 0) AS request_count
      FROM usage_api_daily
      WHERE LOWER(RTRIM(path, '/')) <> '/api/update/latest'
    `).get()?.request_count || 0);
    ...
    const featureEvents = this.database.prepare(`
      ...
      LIMIT 200
    `).all();
    return { devices, downloads, apiRequestTotal, apiRoutes, featureTotals, featureEvents };
  }
  ...
  pruneUsage(retentionDays = RAW_EVENT_RETENTION_DAYS) {
    const cutoffDate = new Date(Date.now() - retentionDays * 24 * 60 * 60 * 1000);
    const dateCutoff = dateKey(cutoffDate);
    const cutoff = cutoffDate.toISOString();
    const api = this.database.prepare('DELETE FROM usage_api_daily WHERE date < ?').run(dateCutoff);
    const downloads = this.database.prepare('DELETE FROM usage_release_downloads WHERE date < ?').run(dateCutoff);
    const features = this.database.prepare('DELETE FROM usage_feature_events WHERE occurred_at < ?').run(cutoff);
    const devices = this.database.prepare('DELETE FROM usage_devices WHERE last_seen_at < ?').run(cutoff);
    return {
      cutoff,
      dateCutoff,
      deleted: {
        api: Number(api.changes),
        downloads: Number(downloads.changes),
        features: Number(features.changes),
        devices: Number(devices.changes)
      }
    };
  }

  removeUsageDevice(deviceKey) {
    if (!deviceKey) return 0;
    const result = this.database.prepare('DELETE FROM usage_devices WHERE device_key = ?').run(deviceKey);
