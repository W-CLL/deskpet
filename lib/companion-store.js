...
const COMPANION_MIGRATIONS = [{
  ...
  up(database) {
    database.exec(`
      ALTER TABLE companion_deliveries ADD COLUMN secret_match INTEGER NOT NULL DEFAULT 0;
      ALTER TABLE companion_daily_stats ADD COLUMN secret_matches INTEGER NOT NULL DEFAULT 0;
      ALTER TABLE companion_daily_stats ADD COLUMN sticker_sent INTEGER NOT NULL DEFAULT 0;
      ALTER TABLE companion_daily_stats ADD COLUMN sticker_received INTEGER NOT NULL DEFAULT 0;
      CREATE TABLE IF NOT EXISTS companion_secrets (
        account_id TEXT PRIMARY KEY,
        secret_date TEXT NOT NULL,
        sha256 TEXT NOT NULL,
        created_at TEXT NOT NULL,
        updated_at TEXT NOT NULL
      );
      CREATE TABLE IF NOT EXISTS companion_stickers (
        id TEXT PRIMARY KEY,
        sender_account_id TEXT NOT NULL,
        recipient_account_id TEXT NOT NULL,
      ...
      CREATE INDEX IF NOT EXISTS idx_companion_stickers_recipient
        ON companion_stickers(recipient_account_id, acknowledged_at, expires_at, created_at);
      CREATE INDEX IF NOT EXISTS idx_companion_stickers_sender
        ON companion_stickers(sender_account_id, created_at DESC);
    `);
  }
}];
...
function mapProfile(row, partner, todaySecretSet) {
  return {
...
function mapDelivery(row, senderName) {
  return {
    ...
    secretMatch: Boolean(row.secret_match),
    downloadPath: `/api/companion/deliveries/${row.id}/file`
  };
}

function mapSticker(row, senderName) {
  return {
    id: row.id,
    senderName,
...
class CompanionStore {
  ...
  profile(accountId) {
    ...
    const partner = row.paired_account_id
      ...
      : null;
    const todaySecretSet = Boolean(this.database.prepare(`
      SELECT 1 FROM companion_secrets WHERE account_id = ? AND secret_date = ?
    `).get(accountId, shanghaiDate()));
    return mapProfile(row, partner, todaySecretSet);
  }
  ...
  unpair(accountId) {
    ...
    try {
      ...
      if (partnerId) {
        ...
        this.database.prepare(`
          UPDATE companion_stickers SET expires_at = ?
          WHERE acknowledged_at IS NULL
            AND (sender_account_id IN (?, ?) OR recipient_account_id IN (?, ?))
        `).run(updatedAt, accountId, partnerId, accountId, partnerId);
      }
  ...
  setSecret(accountId, sha256) {
    const profile = this.ensureProfile(accountId);
    if (!profile.paired_account_id) throw new Error('请先绑定搭子');
    const now = nowIso();
    const date = shanghaiDate(now);
    this.database.prepare(`
      INSERT INTO companion_secrets (account_id, secret_date, sha256, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?)
      ON CONFLICT(account_id) DO UPDATE SET
        secret_date = excluded.secret_date,
        sha256 = excluded.sha256,
        updated_at = excluded.updated_at
    `).run(accountId, date, sha256, now, now);
    return { date, set: true };
  }

  createSticker(senderAccountId, stickerId) {
    if (!Object.hasOwn(STICKER_CATALOG, stickerId)) throw new Error('贴纸无效');
    ...
    const pending = this.database.prepare(`
      SELECT COUNT(*) AS count FROM companion_stickers
      WHERE recipient_account_id = ? AND acknowledged_at IS NULL AND expires_at > ?
    `).get(sender.paired_account_id, createdAt);
    if (Number(pending.count) >= this.maxPending) throw new Error('对方还有未查看的贴纸');
    const id = crypto.randomUUID();
    this.database.prepare(`
      INSERT INTO companion_stickers
        (id, sender_account_id, recipient_account_id, sticker_id, created_at, expires_at)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(id, senderAccountId, sender.paired_account_id, stickerId, createdAt, expiresAt);
    this.database.prepare(`
      INSERT INTO companion_daily_stats (date, sticker_sent)
      VALUES (?, 1)
      ON CONFLICT(date) DO UPDATE SET sticker_sent = sticker_sent + 1
  ...
  createDelivery(senderAccountId, { id, fileName, size, sha256, width, height }) {
    ...
    const createdAt = new Date(now).toISOString();
    const expiresAt = new Date(now + this.deliveryTtlMs).toISOString();
    const secretMatch = Boolean(this.database.prepare(`
      SELECT 1
      FROM companion_secrets sender_secret
      JOIN companion_secrets recipient_secret
        ON recipient_secret.account_id = ?
       AND recipient_secret.secret_date = sender_secret.secret_date
       AND recipient_secret.sha256 = sender_secret.sha256
      WHERE sender_secret.account_id = ?
        AND sender_secret.secret_date = ?
        AND sender_secret.sha256 = ?
    `).get(sender.paired_account_id, senderAccountId, shanghaiDate(createdAt), sha256));
    ...
    try {
      this.database.prepare(`
        INSERT INTO companion_deliveries
          (id, sender_account_id, recipient_account_id, file_name, size, sha256,
           width, height, created_at, expires_at, secret_match)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        id,
        senderAccountId,
      ...
      if (secretMatch) {
        this.database.prepare(`
          INSERT INTO companion_daily_stats (date, secret_matches)
          VALUES (?, 1)
          ON CONFLICT(date) DO UPDATE SET secret_matches = secret_matches + 1
        `).run(shanghaiDate(createdAt));
      }
      this.database.exec('COMMIT');
    } catch (error) {
  ...
  pendingStickers(accountId) {
    this.ensureProfile(accountId);
    const now = nowIso();
    return this.database.prepare(`
      SELECT s.*, p.display_name AS sender_name
      FROM companion_stickers s
      JOIN companion_profiles p ON p.account_id = s.sender_account_id
      WHERE s.recipient_account_id = ? AND s.acknowledged_at IS NULL AND s.expires_at > ?
      ORDER BY s.created_at ASC LIMIT ?
    `).all(accountId, now, this.maxPending).map((row) => mapSticker(row, row.sender_name));
  }
  ...
  acknowledgeSticker(accountId, id) {
    const acknowledgedAt = nowIso();
    this.database.exec('BEGIN IMMEDIATE');
    try {
      const sticker = this.database.prepare(`
        SELECT created_at FROM companion_stickers
        WHERE id = ? AND recipient_account_id = ? AND acknowledged_at IS NULL
      `).get(id, accountId);
      if (!sticker) {
        this.database.exec('COMMIT');
      ...
      const result = this.database.prepare(`
        UPDATE companion_stickers SET acknowledged_at = ?
        WHERE id = ? AND recipient_account_id = ? AND acknowledged_at IS NULL
      `).run(acknowledgedAt, id, accountId);
      if (result.changes === 1) {
        this.database.prepare(`
          INSERT INTO companion_daily_stats (date, sticker_received)
          VALUES (?, 1)
          ON CONFLICT(date) DO UPDATE SET sticker_received = sticker_received + 1
        `).run(shanghaiDate(sticker.created_at));
      }
      this.database.exec('COMMIT');
      return result.changes === 1 ? { id, acknowledgedAt } : null;
    } catch (error) {
  ...
  removeExpired() {
    ...
    try {
      ...
      this.database.prepare(`
        DELETE FROM companion_stickers
        WHERE expires_at <= ? OR (acknowledged_at IS NOT NULL AND acknowledged_at <= ?)
      `).run(currentTime, cutoff);
  ...
  adminStats() {
    ...
    const pending = this.database.prepare(`
      ...
      WHERE acknowledged_at IS NULL AND expires_at > ?
    `).get(now);
    const pendingStickers = this.database.prepare(`
      SELECT COUNT(*) AS count FROM companion_stickers
      WHERE acknowledged_at IS NULL AND expires_at > ?
    `).get(now);
    const totals = this.database.prepare(`
      SELECT
        ...
        COALESCE(SUM(secret_matches), 0) AS secret_matches,
        COALESCE(SUM(sticker_sent), 0) AS sticker_sent,
        COALESCE(SUM(sticker_received), 0) AS sticker_received
      FROM companion_daily_stats
      WHERE date >= ?
    `).get(statsCutoff);
    const daily = this.database.prepare(`
      SELECT date, sent, received, expired, secret_matches, sticker_sent, sticker_received
      FROM companion_daily_stats
      WHERE date >= ?
      ORDER BY date DESC
    `).all(dailyCutoff).map((row) => ({
      ...
      secretMatches: Number(row.secret_matches || 0),
      stickerSent: Number(row.sticker_sent || 0),
      stickerReceived: Number(row.sticker_received || 0)
    }));
    ...
    return {
      ...
      summary: {
        ...
        pendingStickers: Number(pendingStickers.count),
        expired: Number(totals.expired),
        receiptRate: sent > 0 ? received / sent : null,
        secretMatches: Number(totals.secret_matches),
        stickerSent: Number(totals.sticker_sent),
        stickerReceived: Number(totals.sticker_received)
      },
      daily
    };
  }
...
module.exports = { CompanionStore, cleanName, STICKER_CATALOG };
