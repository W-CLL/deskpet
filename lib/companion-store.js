...
const COMPANION_MIGRATIONS = [{
  ...
  version: 6,
  name: 'companion-delivery-source',
  up(database) {
    const deliveryColumns = new Set(
      database.prepare('PRAGMA table_info(companion_deliveries)').all().map((row) => row.name)
    );
    if (!deliveryColumns.has('source')) {
      database.exec("ALTER TABLE companion_deliveries ADD COLUMN source TEXT NOT NULL DEFAULT 'pair'");
    }
  }
}];
...
class CompanionStore {
  ...
  ensureProfile(accountId) {
    ...
    this.database.prepare(`
      INSERT INTO companion_profiles
        (account_id, display_name, pairing_code, hall_enabled, last_seen_at, created_at, updated_at)
      VALUES (?, '桌搭子', ?, 0, NULL, ?, ?)
  ...
  promoteTrialProfile(installationId, accountId, licenseId) {
    const trialId = `trial:${installationId}`;
    const trial = this.database.prepare('SELECT * FROM companion_profiles WHERE account_id = ?').get(trialId);
    if (!trial) return;
    this.database.exec('BEGIN IMMEDIATE');
    try {
      const existing = this.database.prepare('SELECT 1 FROM companion_profiles WHERE account_id = ?').get(accountId);
      this.ensureProfile(accountId);
      // A second device must not overwrite the account's existing hall choice.
      if (!existing) {
        this.database.prepare(`
          UPDATE companion_profiles SET display_name = ?, hall_enabled = ?, last_seen_at = ?, updated_at = ?
          WHERE account_id = ?
        `).run(trial.display_name, trial.hall_enabled, trial.last_seen_at, nowIso(), accountId);
      }
      this.database.prepare('UPDATE companion_deliveries SET sender_account_id = ? WHERE sender_account_id = ?')
        .run(accountId, trialId);
      this.database.prepare('UPDATE companion_deliveries SET recipient_account_id = ? WHERE recipient_account_id = ?')
        .run(accountId, trialId);
      this.database.prepare(`
        INSERT OR IGNORE INTO companion_delivery_receipts (delivery_id, license_id, acknowledged_at)
        SELECT delivery_id, ?, acknowledged_at FROM companion_delivery_receipts WHERE license_id = ?
      `).run(licenseId, trialId);
      this.database.prepare('DELETE FROM companion_profiles WHERE account_id = ?').run(trialId);
  ...
  pair(accountId, inputCode) {
    ...
    try {
      ...
      if (!partner || partner.account_id === accountId
        || partner.account_id.startsWith('trial:') || accountId.startsWith('trial:')) {
        throw new Error('搭子码无效');
      }
  ...
  createHallDelivery(senderAccountId, recipientAccountId, metadata) {
    ...
    return this.createDeliveryFor(senderAccountId, recipientAccountId, { ...metadata, source: 'hall' });
  }

  createAdminDelivery(senderAccountId, recipientAccountId, metadata) {
    if (!senderAccountId || senderAccountId === recipientAccountId) {
      throw new Error('不能发给自己');
    }
    this.ensureProfile(senderAccountId);
    this.ensureProfile(recipientAccountId);
    return this.createDeliveryFor(senderAccountId, recipientAccountId, { ...metadata, source: 'admin' });
  }

  createDeliveryFor(senderAccountId, recipientAccountId, {
    id, fileName, size, sha256, width, height, message = '', source = 'pair'
  }) {
    const now = Date.now();
    const createdAt = new Date(now).toISOString();
    const expiresAt = new Date(now + this.deliveryTtlMs).toISOString();
    const deliverySource = source === 'hall' || source === 'admin' ? source : 'pair';
    if (deliverySource !== 'admin') {
      ...
      `).get(senderAccountId);
      if (recent && Date.parse(recent.created_at) > now - this.cooldownMs) {
        throw new Error('发送太快，请稍后再试');
      }
    }
      ...
      this.database.prepare(`
        INSERT INTO companion_deliveries
          (id, sender_account_id, recipient_account_id, file_name, size, sha256,
           width, height, message, source, created_at, expires_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        id,
        senderAccountId,
    ...
    return {
      id,
      recipientName: this.ensureProfile(recipientAccountId).display_name,
      createdAt,
      expiresAt
    };
  }
  ...
  displayNameByAccountId() {
    ...
    for (const row of this.database.prepare(`
      SELECT account_id, display_name FROM companion_profiles
    `).all()) {
      map.set(row.account_id, row.display_name || '桌搭子');
    }
    return map;
  }
  ...
  adminStats() {
    ...
    const profileRows = this.database.prepare(`
      SELECT p.account_id, p.display_name, p.hall_enabled, p.last_seen_at,
        p.created_at, p.updated_at, partner.account_id AS partner_account_id,
        partner.display_name AS partner_name, p.paired_at
      FROM companion_profiles p
      LEFT JOIN companion_profiles partner ON partner.account_id = p.paired_account_id
    ...
    `).all().map((row) => ({
      ...
      partner: row.partner_account_id ? {
        ...
        accountSuffix: String(row.partner_account_id).slice(-8),
        displayName: row.partner_name,
        pairedAt: row.paired_at
      } : null
    }));
    const pairs = this.database.prepare(`
      SELECT first.account_id AS first_account_id, first.display_name AS first_name,
        second.account_id AS second_account_id, second.display_name AS second_name,
        first.paired_at
      FROM companion_profiles first
      JOIN companion_profiles second ON second.account_id = first.paired_account_id
      WHERE first.account_id < second.account_id
      ORDER BY first.paired_at DESC
    `).all().map((row) => ({
      firstAccountId: row.first_account_id,
      firstAccountSuffix: String(row.first_account_id).slice(-8),
      firstName: row.first_name,
      secondAccountId: row.second_account_id,
      secondAccountSuffix: String(row.second_account_id).slice(-8),
      secondName: row.second_name,
      pairedAt: row.paired_at
    }));
    const recentDeliveries = this.database.prepare(`
      SELECT delivery.id, delivery.source, delivery.message, delivery.size,
        delivery.created_at, delivery.expires_at, delivery.acknowledged_at,
        sender.account_id AS sender_account_id, sender.display_name AS sender_name,
        recipient.account_id AS recipient_account_id, recipient.display_name AS recipient_name
      FROM companion_deliveries delivery
      JOIN companion_profiles sender ON sender.account_id = delivery.sender_account_id
      JOIN companion_profiles recipient ON recipient.account_id = delivery.recipient_account_id
    ...
    `).all().map((row) => ({
      ...
      status: row.acknowledged_at ? 'received' : row.expires_at <= now ? 'expired' : 'pending',
      sender: {
        accountId: row.sender_account_id,
        accountSuffix: String(row.sender_account_id).slice(-8),
        displayName: row.sender_name
      },
      recipient: {
        accountId: row.recipient_account_id,
        accountSuffix: String(row.recipient_account_id).slice(-8),
