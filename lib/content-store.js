...
const { CONTENT_TYPES } = require('./content-types');
...
const CONTENT_MIGRATIONS = [
  ...
  {
    version: 2,
    name: 'expand-content-types',
    up(database) {
      database.exec(`
        CREATE TABLE content_items_next (
          id TEXT PRIMARY KEY,
          type TEXT NOT NULL CHECK (type IN ('joke', 'math', 'trivia', 'riddle', 'tip', 'care')),
          revision INTEGER NOT NULL DEFAULT 1 CHECK (revision >= 1),
          prompt TEXT NOT NULL,
          answer TEXT NOT NULL,
          explanation TEXT NOT NULL DEFAULT '',
        ...
        SELECT id, type, revision, prompt, answer, explanation, choices_json,
          tags_json, difficulty, locale, status, created_at, updated_at
        FROM content_items;
...
class ContentStore {
  ...
  disableMany({ ids = [], type = '' } = {}) {
    ...
    try {
      ...
      let disabled;
      if (type) {
        requested = Number(this.database.prepare(`
          SELECT COUNT(*) AS total FROM content_items WHERE type = ?
        `).get(type).total || 0);
        disabled = Number(this.database.prepare(`
          UPDATE content_items
          SET status = 'disabled', revision = revision + 1, updated_at = ?
          WHERE type = ? AND status = 'active'
        `).run(disabledAt, type).changes);
      } else {
        ...
        `).run(disabledAt, ...ids).changes);
      }
      const changed = disabled > 0;
      const catalog = changed ? this.bumpCatalog(disabledAt) : this.catalog();
      this.database.exec('COMMIT');
      return {
        mode: type ? 'type' : 'ids',
        ...(type ? { type } : {}),
  ...
  listAll() {
    ...
    const counts = this.database.prepare(`
      SELECT COUNT(*) AS total,
        ...
        SUM(CASE WHEN type = 'riddle' AND status = 'active' THEN 1 ELSE 0 END) AS riddles,
        SUM(CASE WHEN type = 'tip' AND status = 'active' THEN 1 ELSE 0 END) AS tips,
        SUM(CASE WHEN type = 'care' AND status = 'active' THEN 1 ELSE 0 END) AS care
      FROM content_items
    `).get();
    return {
      ...
      summary: {
        ...
        riddles: Number(counts.riddles || 0),
