...
class InteractionStore {
  constructor(dataDirectory, { getDefaults = () => ({}) } = {}) {
  ...
  async initialize() {
    const opened = await openMigratedDatabase({
      ...
      fileName: 'interaction.db',
      scope: INTERACTION_MIGRATION_SCOPE,
      migrations: INTERACTION_MIGRATIONS,
      foreignKeys: true
    });
  ...
  ensureAccount(accountId, createdAt = nowIso()) {
    const requestedMode = this.getDefaults()?.interactionMode;
    const mode = INTERACTION_MODES.includes(requestedMode) ? requestedMode : 'standard';
    this.database.prepare(`
      INSERT OR IGNORE INTO interaction_profiles
        (account_id, mode, prompts_enabled, created_at, updated_at)
      VALUES (?, ?, 1, ?, ?)
    `).run(accountId, mode, createdAt, createdAt);
    this.database.prepare(`
