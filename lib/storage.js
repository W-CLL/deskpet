...
function normalizeXianyuUrl(value) {
  const raw = String(value || '').trim();
  if (!raw) return '';
  if (raw.length > 500) throw new Error('闲鱼链接不能超过 500 个字符');
  let url;
  try {
    url = new URL(raw);
  } catch {
    throw new Error('闲鱼链接格式无效');
  }
  const hostname = url.hostname.toLowerCase();
  const allowed = hostname === 'm.tb.cn'
    || hostname === '2.taobao.com'
    || hostname === 'goofish.com'
    || hostname.endsWith('.goofish.com');
  if (url.protocol !== 'https:' || !allowed) throw new Error('请输入有效的闲鱼 HTTPS 链接');
  url.hash = '';
  return url.href;
}
...
const PERSONALITIES = new Set(['lively', 'shy', 'clingy', 'chaotic']);
const INTERACTION_MODES = new Set(['quiet', 'standard', 'lively']);
const THEATER_INTERVALS = new Set([60, 180, 300, 600, 1800]);
...
function normalizeWechatId(value, fallback = DEFAULT_SITE_SETTINGS.wechatId) {
  const raw = String(value == null ? fallback : value).trim();
  if (!raw) return '';
  if (raw.length > 32) throw new Error('微信号不能超过 32 个字符');
  if (!/^[A-Za-z][-_A-Za-z0-9]{5,19}$/.test(raw)) throw new Error('请输入有效的微信号');
  return raw;
}
...
function normalizeFeatures(value) {
  ...
  return {
    trialVisits: asBoolean(source.trialVisits, true),
    companionHall: asBoolean(source.companionHall, true),
    fishMode: asBoolean(source.fishMode, true),
    autoUpdates: asBoolean(source.autoUpdates, true)
  };
}

function normalizeDefaults(value) {
  const source = value && typeof value === 'object' ? value : {};
  const personality = String(source.personality || DEFAULT_SITE_SETTINGS.defaults.personality);
  const interactionMode = String(source.interactionMode || DEFAULT_SITE_SETTINGS.defaults.interactionMode);
  const theaterIntervalSeconds = Number(source.theaterIntervalSeconds);
  return {
    personality: PERSONALITIES.has(personality) ? personality : DEFAULT_SITE_SETTINGS.defaults.personality,
    interactionMode: INTERACTION_MODES.has(interactionMode)
      ? interactionMode
      : DEFAULT_SITE_SETTINGS.defaults.interactionMode,
    theaterIntervalSeconds: THEATER_INTERVALS.has(theaterIntervalSeconds)
      ? theaterIntervalSeconds
      : DEFAULT_SITE_SETTINGS.defaults.theaterIntervalSeconds
  };
}
...
function normalizeSiteSettings(value) {
  const source = value && typeof value === 'object' ? value : {};
  return {
    xianyuUrl: normalizeXianyuUrl(source.xianyuUrl),
    wechatId: normalizeWechatId(source.wechatId),
...
function expectedReleaseFileName(platformValue, architectureValue, versionValue) {
  ...
  if (platform === 'android') {
    const architectureLabel = architecture === 'arm64-v8a' ? 'arm64' : 'armv7';
    return `ZhuoDazi-Android-${version}-${architectureLabel}.apk`;
  }
...
class ReleaseStore {
  constructor(dataDirectory) {
    ...
    this.data = {
      ...
      activeVersions: {},
      publicVersions: {},
      siteSettings: { ...DEFAULT_SITE_SETTINGS, features: { ...DEFAULT_SITE_SETTINGS.features }, defaults: { ...DEFAULT_SITE_SETTINGS.defaults } },
      releases: []
    };
  ...
  async initialize() {
    ...
    try {
      ...
      if (!legacy && ![2, 3].includes(parsed.schemaVersion)) throw new Error('版本数据格式无效');
      ...
      const publicVersions = !legacy && parsed.publicVersions
        ? Object.fromEntries(Object.entries(parsed.publicVersions)
          .filter(([key, version]) => typeof version === 'string' && /^\w+\/[-_\w]+$/.test(key))
          .map(([key, version]) => [key, normalizeVersion(version)]))
        : { ...activeVersions };
      const siteSettings = normalizeSiteSettings(parsed.siteSettings || {});
      this.data = { schemaVersion: 3, activeVersions, publicVersions, siteSettings, releases };
      const missingRemoteConfig = !parsed.siteSettings
        || typeof parsed.siteSettings.features !== 'object'
        || typeof parsed.siteSettings.defaults !== 'object'
        || !Object.hasOwn(parsed.siteSettings, 'wechatId');
      if (legacy || parsed.schemaVersion !== 3 || !parsed.publicVersions || missingRemoteConfig) {
  ...
  list() {
    return this.data.releases
      .map((release) => ({
        ...
        public: release.version === this.data.publicVersions[releaseKey(release.platform, release.architecture)]
      }))
  ...
  async publish(platform, architecture, version) {
    return this.serializeMutation(async () => {
      ...
      const previous = {
        activeVersion: this.data.activeVersions[key],
        publicVersion: this.data.publicVersions[key],
        publishedAt: release.publishedAt
      };
      release.publishedAt = release.publishedAt || new Date().toISOString();
      this.data.activeVersions[key] = release.version;
      this.data.publicVersions[key] = release.version;
      try {
        await atomicWriteJson(this.metadataPath, this.data);
      } catch (error) {
        ...
        if (previous.publicVersion) this.data.publicVersions[key] = previous.publicVersion;
        else delete this.data.publicVersions[key];
        throw error;
      }
  ...
  public(platform, architecture) {
    const key = releaseKey(platform, architecture);
    const version = this.data.publicVersions[key] || this.data.activeVersions[key];
    return version ? this.find(platform, architecture, version) : null;
  }
  ...
  async updateSiteSettings(value) {
    return this.serializeMutation(async () => {
      const previous = this.data.siteSettings;
      this.data.siteSettings = normalizeSiteSettings({
        ...previous,
      ...
      try {
        ...
        this.data.siteSettings = previous;
        throw error;
      }
