...
class ReleaseStore {
  ...
  async initialize() {
    ...
    try {
      ...
      const publicVersions = !legacy && parsed.publicVersions
        ...
        : { ...activeVersions };
      const siteSettings = {
        xianyuUrl: normalizeXianyuUrl(parsed.siteSettings?.xianyuUrl)
      };
      this.data = { schemaVersion: 3, activeVersions, publicVersions, siteSettings, releases };
      if (legacy || parsed.schemaVersion !== 3 || !parsed.publicVersions || !parsed.siteSettings) {
  ...
  async updateSiteSettings(value) {
    return this.serializeMutation(async () => {
      ...
      this.data.siteSettings = { xianyuUrl: normalizeXianyuUrl(value) };
      try {
        await atomicWriteJson(this.metadataPath, this.data);
      } catch (error) {
