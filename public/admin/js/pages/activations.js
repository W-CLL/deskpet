registerAdminPage(function createActivationsPage({ ui, api, showToast, confirmAction, navigateTo }) {
  ...
  function openGenerateDrawer() {
    const form = byId('generateCodeForm');
    globalThis.AdminFormKit.openFormDrawer({
      ...
      onCancel: () => {
        ...
        byId('activationCount').value = '1';
        byId('activationExpiry').value = '30';
      },
      focusSelector: '#activationCount'
    });
  }
  ...
  bindSubmit('generateCodeForm', async () => {
    ...
    await withBusy(button, async () => {
      ...
      byId('generatedCodesDialog').showModal();
      byId('activationNote').value = '';
      byId('generateCodeForm').hidden = true;
  ...
  bindClick('createActivationButton', openGenerateDrawer);

  return { id: 'activations', load: loadActivations };
});
