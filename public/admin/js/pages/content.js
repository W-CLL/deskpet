registerAdminPage(function createContentPage({ ui, api, showToast, confirmAction }) {
  ...
  function openContentDrawer(mode) {
    const form = byId('contentEditorForm');
    if (!form || !globalThis.AdminFormKit?.openFormDrawer) {
      form?.scrollIntoView({ block: 'start', behavior: 'smooth' });
      return;
    }
    globalThis.AdminFormKit.openFormDrawer({
      form,
      title: mode === 'edit' ? '编辑内容' : '新增内容',
      eyebrow: '内容库',
      submitLabel: mode === 'edit' ? '保存修改' : '新增内容',
      onCancel: resetContentEditor,
      focusSelector: '#contentPrompt'
    });
  }
  ...
  function renderContent(payload) {
    ...
    setText('contentActive', active);
    setText('contentActiveRate', total ? `${Math.round((active / total) * 1000) / 10}%` : '0%');
  ...
  bindSubmit('contentEditorForm', async () => {
    const button = document.querySelector('#editorDrawer .drawer-footer-actions .button-primary')
      || byId('saveContentButton');
    await withBusy(button, async () => {
      ...
      resetContentEditor();
      byId('contentEditorForm').hidden = true;
      globalThis.AdminFormKit?.closeDrawer();
      await loadContent();
    });
  });
  bindClick('cancelContentEditButton', () => {
    resetContentEditor();
    byId('contentEditorForm').hidden = true;
    globalThis.AdminFormKit?.closeDrawer();
  });
  bindClick('createContentButton', () => {
    resetContentEditor();
    openContentDrawer('create');
  });
  bindClick('openImportContentButton', () => {
    const form = byId('contentImportForm');
    globalThis.AdminFormKit.openFormDrawer({
      form,
      title: '批量导入内容',
      eyebrow: '内容库',
      submitLabel: '开始导入',
      onCancel: () => form.reset(),
      focusSelector: '#contentImportFile'
    });
  });
  ...
  bindSubmit('contentImportForm', async () => {
    ...
    await withBusy(button, async () => {
      ...
      byId('contentImportForm').hidden = true;
      globalThis.AdminFormKit?.closeDrawer();
      resetContentEditor();
      await loadContent();
    }, (error) => {
  ...
  return { id: 'content', load: loadContent };
});
