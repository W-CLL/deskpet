registerAdminPage(function createVisitStickersPage({ ui, api, showToast, confirmAction }) {
  ...
  function openUploadDrawer() {
    const form = byId('visitStickerForm');
    globalThis.AdminFormKit.openFormDrawer({
      ...
      submitLabel: '上传并启用',
      onCancel: () => form.reset(),
      focusSelector: '#visitStickerTitle'
    });
  }
  ...
  bindSubmit('visitStickerForm', () => submitUpload({
    ...
    afterReset: () => {
      byId('visitStickerForm').hidden = true;
  ...
  bindClick('createVisitStickerButton', openUploadDrawer);

  return { id: 'visit-stickers', load: loadVisitStickers };
});
