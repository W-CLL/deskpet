...
test('admin shell includes the Android management hooks used by admin.js', async () => {
  ...
  assert.match(adminMarkup, /admin-ui\.js\?v=admin-pages-2/);
  assert.match(adminMarkup, /admin-pages\/releases\.js\?v=admin-pages-2/);
  assert.match(adminMarkup, /admin\.js\?v=admin-pages-2/);
...
test('admin upload, publish, manifest and download workflow', async (context) => {
  ...
  const adminUi = await fetch(`${baseUrl}/assets/admin-ui.js?v=admin-pages-2`);
  ...
  const releasesPage = await fetch(`${baseUrl}/assets/admin-pages/releases.js?v=admin-pages-2`);
